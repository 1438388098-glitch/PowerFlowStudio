...
(纯析构顺序问题, 用例本身全过)。仅在 Windows 上于 pytest 完全收尾
(unconfigure, 摘要已打印)后硬退出避开析构, 并向 stderr 打印
TEARDOWN CRASH SUPPRESSED 标记以便识别; 退出码沿用 pytest 的判定,
失败时为 1。其他平台不干预, 正常走解释器关闭流程。
"""
import os
import sys
...
def pytest_unconfigure(config):
    ...
    try:
        ...
        pass
    print("TEARDOWN CRASH SUPPRESSED: Windows PyQt5/PyQtGraph destructor "
          "workaround (tests finished, skipping interpreter shutdown)",
          file=sys.stderr)
    sys.stderr.flush()
